...
spawn(function() 
    ...
    wait(1.5)
    require(game.ReplicatedStorage.Notification).new("<Color=Red>NeyoshiiHub<Color=/>"):Display()
    wait(.5)
    ...
    wait(.29)
    require(game.ReplicatedStorage.Notification).new("<Color=Yellow>Theo dõi Facebook admin để update tình hình mới nhất nhé!<Color=/>"):Display()
    wait(.36)
end)
...
W:AddLabel("Neyoshii")
W:AddLabel("Discord NeyoshiiHub")
local Section = W:AddSection({
    Name = "Status"
})
...
C:AddButton({
    ...
    Callback = function(value)
        setclipboard("https://discord.gg/FPbeNcDqKD")
    end
})
...
pcall(function()
    while wait() do
        if _G.AutoEliteHunter then
            if game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible == true then
                OrionLib:MakeNotification({
                    Name = "NeyoshiiHub (Noitification)",
